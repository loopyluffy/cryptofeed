...
# customized with prefix Loopy by @logan
cdef class LoopyBalance(Balance):
    ...
    cpdef dict to_dict(self, numeric_type=None, none_to=False):
        if numeric_type is None:
            data =  {'exchange': self.exchange, 'currency': self.currency, 'symbol': self.currency, 'balance': self.balance, 'cw_balance': self.cw_balance, 'reserved': self.reserved, 'changed': self.changed, 'timestamp': self.timestamp}
        else:
            data = {'exchange': self.exchange, 'currency': self.currency, 'symbol': self.currency, 'balance': numeric_type(self.balance), 'cw_balance': numeric_type(self.cw_balance), 'reserved': numeric_type(self.reserved), 'changed': self.changed, 'timestamp': self.timestamp}
        return data if not none_to else convert_none_values(data, none_to)        
...
cdef class LoopyPosition:
    ...
    def __init__(self, exchange, symbol, margin_type, side, entry_price, amount, unrealised_pnl, cum_pnl, timestamp, raw=None):
        assert isinstance(entry_price, Decimal)
        assert isinstance(amount, Decimal)
        assert isinstance(unrealised_pnl, Decimal)
        assert isinstance(cum_pnl, Decimal)
        assert timestamp is None or isinstance(timestamp, float)
    ...
    cpdef dict to_dict(self, numeric_type=None, none_to=False):
        if numeric_type is None:
            data = {'exchange': self.exchange, 'symbol': self.symbol, 'margin_type': self.margin_type, 'side': self.side, 'entry_price': self.entry_price, 'amount': self.amount, 'unrealised_pnl': self.unrealised_pnl, 'cum_pnl': self.cum_pnl, 'timestamp': self.timestamp}
        else:
            data = {'exchange': self.exchange, 'symbol': self.symbol, 'margin_type': self.margin_type, 'side': self.side, 'entry_price': numeric_type(self.entry_price), 'amount': numeric_type(self.amount), 'unrealised_pnl': numeric_type(self.unrealised_pnl), 'cum_pnl': numeric_type(self.cum_pnl), 'timestamp': self.timestamp}
        return data if not none_to else convert_none_values(data, none_to)
    ...
    def __eq__(self, cmp):
        return self.exchange == cmp.exchange and self.symbol == cmp.symbol and self.margin_type == cmp.margin_type and self.side == cmp.side and self.entry_price == cmp.entry_price and self.amount == cmp.amount and self.unrealised_pnl == cmp.unrealised_pnl and self.cum_pnl == cmp.cum_pnl and self.timestamp == cmp.timestamp
